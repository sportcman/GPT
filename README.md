...
### Назначения скрипта: Мастер тренеровок JSON с параметрами prompt и response, и процесс использования.

## Описание
Этот скрипт создан для обучения модели GPT на основе предоставленного датасета, с использованием графического интерфейса на PyQt6. Пользователь может выбрать модель и датасет, задать параметры обучения и наблюдать за процессом в реальном времени. Основная цель — упростить процесс обучения модели на локальной машине, при этом сохраняя возможность настройки важных параметров через удобный интерфейс.

## Основные функции:
Выбор модели и датасета: Через интерфейс можно выбрать модель и датасет в формате JSON, содержащий данные для обучения.
Настройка параметров обучения: В интерфейсе предоставлены поля для настройки параметров:
Batch Size (размер партии)
Epochs (количество эпох)
Gradient Accumulation Steps (шаги накопления градиента)
Block Size (размер фрагмента текста)
Опция перекрытия фрагментов с указанием размера перекрытия
Визуализация прогресса: Прогресс обучения отображается с помощью полосы прогресса, а логи выводятся в текстовом поле в реальном времени, где указаны потери (loss), количество завершённых эпох и обработанных батчей.
Остановка и сохранение: Скрипт позволяет прервать обучение в любой момент, при этом модель и токенизатор будут сохранены на диск.
Компоненты скрипта
1. Класс CustomTextDataset
Этот класс предназначен для работы с текстовыми данными в формате JSON. Он обрабатывает пары «вопрос-ответ» и подготавливает данные для обучения модели, разбивая их на фрагменты (блоки) с возможностью перекрытия между ними.

2. Класс TrainingWorker
Это поток, в котором выполняется основная часть обучения. Поток обеспечивает выполнение всех вычислительных операций в фоновом режиме, позволяя интерфейсу оставаться отзывчивым. В этом классе реализована логика обучения, накопления градиентов, сохранения модели и обработки ошибок.

3. Главный интерфейс (MainWindow)
Пользователь взаимодействует с этим окном для:

Выбора модели и датасета
Настройки параметров обучения
Запуска и остановки процесса обучения
Инструкция по использованию
1. Запуск программы
Для запуска программы выполните команду:

Убедитесь, что у вас установлен необходимый стек библиотек (PyQt6, torch, transformers, и другие зависимости, указанные в скрипте).

2. Выбор модели и датасета
Нажмите кнопку «Выбрать модель» и укажите путь к папке, содержащей предварительно обученную модель GPT2.
Нажмите кнопку «Выбрать датасет» и выберите JSON-файл, содержащий данные для обучения (формат должен включать поля prompt и response).
3. Настройка параметров
Batch Size: Задайте размер партии, который определяет, сколько примеров будет использоваться для одной итерации обучения.
Epochs: Установите количество эпох, что определяет, сколько раз модель будет проходить через весь датасет.
Gradient Accumulation Steps: Шаги накопления градиента позволяют уменьшить использование памяти, что особенно полезно при обучении на больших датасетах.
Block Size: Определяет размер фрагментов текста, на которые будет разбит датасет для обучения.
Перекрытие фрагментов: Включите флажок, если нужно, чтобы фрагменты перекрывались, и укажите размер перекрытия.
4. Запуск обучения
После выбора модели, датасета и настройки параметров нажмите кнопку «Начать обучение». В процессе обучения вы увидите обновление прогресса и логов в реальном времени. По завершении обучения или его остановке модель и токенизатор будут автоматически сохранены.

6. Остановка обучения
Если вам нужно прервать процесс, нажмите кнопку «Остановить обучение». Обучение будет остановлено, а текущая версия модели и токенизатора сохранится на диск.

Заключение
Этот скрипт является мощным инструментом для локального обучения моделей GPT. Он предоставляет гибкий интерфейс для настройки и управления процессом обучения, упрощает работу с моделью и позволяет контролировать каждый этап процесса.


## ## GPT Текстовый генератор и озвучка описание скрипта.

Назначение
Этот скрипт предназначен для генерации текста с помощью модели GPT-2 и последующей озвучки сгенерированного текста на различных языках. Пользователи могут вводить вопросы, управлять параметрами генерации (такими как температура и количество токенов), а также изменять тембр голоса для синтеза речи. Программа предоставляет удобный графический интерфейс, что делает её доступной даже для пользователей без глубоких технических знаний.

Основные функции
Генерация текста с использованием GPT-2: Пользователь вводит вопрос или текстовый запрос, а модель GPT-2 на основе выбранных параметров генерирует ответ.
Настройка параметров генерации: Пользователь может изменить температуру (управляющую креативностью модели), длину текста (количество генерируемых токенов), а также другие параметры генерации.
Озвучка текста: Программа поддерживает преобразование текста в речь с использованием Google Text-to-Speech (gTTS) и предоставляет возможность выбора языка для озвучивания. Пользователь также может изменить тембр голоса с помощью изменения частоты звука.
Сохранение результата: Сгенерированный текст можно сохранить в текстовый файл для дальнейшего использования.
Пользовательский интерфейс: Удобный интерфейс на базе PyQt6 позволяет легко взаимодействовать с программой, настраивать параметры и управлять процессом генерации текста и озвучки.
Инструкция по использованию
1. Установка модели GPT-2
Для начала работы необходимо выбрать директорию с предобученной моделью GPT-2. Нажмите кнопку "Выбрать модель" и укажите путь к модели, скачанной с ресурсов, поддерживающих модели GPT-2 (например, Hugging Face).

2. Ввод текста для генерации
В поле "Вопрос" введите текстовый запрос, на который хотите получить ответ. Например, вы можете ввести вопрос "Что такое нейронная сеть?".

3. Настройка параметров генерации
Температура: Ползунок температуры управляет "креативностью" модели. Чем выше значение, тем более случайными могут быть результаты. Настройте температуру с помощью ползунка, и текущее значение будет отображаться внизу.
Количество токенов: Вращайте счётчик, чтобы установить количество токенов, которые будут сгенерированы. Чем больше значение, тем длиннее будет ответ.
4. Генерация текста
Нажмите кнопку "Сгенерировать ответ", чтобы получить результат. Сгенерированный текст отобразится в текстовом поле ниже.

5. Сохранение результата
После генерации текста вы можете сохранить его в файл. Для этого нажмите кнопку "Сохранить в файл", выберите место сохранения и имя файла. Текст будет сохранён в формате .txt.

6. Озвучка текста
Для преобразования текста в речь:

Выберите язык озвучки в выпадающем списке (доступны русские, английские и другие популярные языки).
Настройте тембр голоса, изменив положение ползунка "Тембр (голоса)".
Нажмите кнопку "Озвучить ответ", и программа сгенерирует аудиофайл и воспроизведёт его.
7. Завершение работы
После использования программы вы можете закрыть её, нажав на кнопку "Закрыть" или просто закрыв окно.

Возможности и перспективы
Этот скрипт может быть полезен для различных целей, таких как генерация креативных ответов, создание учебных материалов, написание статей и озвучка текстов.
