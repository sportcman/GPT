...
class CustomTextDataset(torch.utils.data.Dataset):
    def __init__(self, tokenizer, file_path, block_size, overlap_size=0):
        ...
        # Создание пар prompt и response
        for item in data:
            prompt = item["prompt"]
...
class MainWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.gradient_accumulation_input.setValue(1)
        ...
        self.overlap_size_input.setValue(32)
    ...
    def start_training(self):
        model_path = self.model_label.text().split(": ")[1]
        dataset_path = self.dataset_label.text().split(": ")[1]
    ...
    def select_dataset(self):
        dataset_path, _ = QFileDialog.getOpenFileName(self, "Выберите файл датасета", "", "JSON Files (*.json)")
        if dataset_path:
